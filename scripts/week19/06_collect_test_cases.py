...
BACKBONE = ROOT / "dataset/setting_b/08_n2_eval_test/test_backbone_raw_eval.json"
ONTOLOGY = ROOT / "dataset/setting_b/08_n2_eval_test/test_ontology_raw_eval.json"
SOFT = ROOT / "dataset/setting_b/08_n2_eval_test/test_soft_support_raw_eval.json"
RETR = ROOT / "dataset/setting_b/08_n2_eval_test/test_retrieval_main_eval.json"
RETR_ARTIFACT = ROOT / "dataset/setting_a/27_n2_fuzzy_retrieval/test_fuzzy_retrieval_main.json"
ABLATION = ROOT / "results/week19/test_ablation.json"

OUT_JSON = ROOT / "results/week19/test_case_shortlist.json"
OUT_MD = ROOT / "reports/week19/day6_test_case_review.md"
...
def main():
    ...
    encoder_bucket = [{
        "bucket_name": "encoder_appendix_deferred",
        "encoder_probe_status": ablation.get("appendix_note", {}).get("encoder_probe_status"),
        "encoder_probe_not_in_main_table": ablation.get("appendix_note", {}).get("encoder_probe_not_in_main_table"),
        "note": (
            "Encoder remains appendix-only on locked test. "
            "No promoted test-side encoder row is shortlisted for the main paper."
        ),
    }]
    ...
    shortlist = {
        ...
        "shortlist": {
            ...
            "encoder_appendix_deferred": encoder_bucket,
        },
    }
    ...
    md = []
    md.append("# Day 6 — Test case review shortlist")
    md.append("")
    ...
    md.append("")
    md.append("## 5. encoder_appendix_deferred")
    md.append("- Encoder remains appendix-only on locked test; no promoted encoder row is shortlisted.")
    md.append("")
    md.append("## 6. Day-6 conclusion")
    md.append(
        "Built test-side shortlist for main-paper interpretation and appendix. "
        "The shortlist now separates ranking improvement cases, ontology failure recovery cases, "
