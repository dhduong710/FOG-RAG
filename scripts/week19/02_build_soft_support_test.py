...
RAW_PATH = ROOT / "dataset/setting_a/23_noinj_source/test_top20_raw.json"
TYPE_FILTERED_PATH = ROOT / "dataset/setting_a/24_noinj_ontology/test_top20_type_filtered_raw.json"
ONTOLOGY_RAW_PATH = ROOT / "dataset/setting_a/24_noinj_ontology/test_top20_ontology_raw.json"
ALIGNED_EVIDENCE_PATH = ROOT / "dataset/setting_a/24b_noinj_evidence/test_aligned_evidence.json"

TYPE_MAP_PATH = ROOT / "dataset/setting_b/01_annotations/type_map.tsv"
SCHEMA_RULES_PATH = ROOT / "dataset/setting_b/01_annotations/schema_rules.json"
TEST_B_PATH = ROOT / "dataset/setting_b/04_contra_checked/test_b_annotations_contra_checked.json"

VALID_MAIN_PATH = ROOT / "dataset/setting_a/26_n2_soft_support/valid_top20_soft_support_main.json"
OUT_PATH = ROOT / "dataset/setting_a/26_n2_soft_support/test_top20_soft_support_main.json"

SUMMARY_PATH = ROOT / "results/week19/soft_support_test_build_summary.json"
REPORT_PATH = ROOT / "reports/week19/day2_build_soft_support_test.md"

# Frozen Formula-B main row from week14 selection
FROZEN_FORMULA = {
...
def compute_score(c):
    ...
    if FROZEN_FORMULA["use_capped_evidence"]:
        cap = float(FROZEN_FORMULA["evidence_cap"])
    ...
    score = (
        float(FROZEN_FORMULA["evidence_positive_weight"]) * evidence_term
        - float(FROZEN_FORMULA["direct_link_penalty"]) * direct_penalty
        - float(FROZEN_FORMULA["contra_penalty_weight"]) * contra
    )
...
def main():
    ...
    frozen_variant_name = valid_variant_name
    ...
    for raw in raw_rows:
        ...
        out_row = {
            ...
            "variant_name": frozen_variant_name,
    ...
    summary = {
        ...
        "variant_name_written_to_test": frozen_variant_name,
        ...
        "policy_checks": {
            ...
            "row_naming_changed": valid_variant_name != frozen_variant_name,
        },
    }
    ...
    md = []
    md.append("# Day 2 — Build soft_support_raw_test")
    md.append("")
    md.append(f"- status: **{summary['status']}**")
    md.append(f"- output: `{OUT_PATH}`")
    md.append(f"- variant_name: `{frozen_variant_name}`")
    md.append("")
    md.append("## 1. Frozen formula")
    md.append(f"- score_family: `{FROZEN_FORMULA['score_family']}`")
    md.append(f"- evidence_positive_weight: `{FROZEN_FORMULA['evidence_positive_weight']}`")
    md.append(f"- direct_link_penalty: `{FROZEN_FORMULA['direct_link_penalty']}`")
    md.append(f"- contra_penalty_weight: `{FROZEN_FORMULA['contra_penalty_weight']}`")
    md.append(f"- use_capped_evidence: `{FROZEN_FORMULA['use_capped_evidence']}`")
    md.append("")
    ...
    for k, v in summary["policy_checks"].items():
        md.append(f"- {k}: `{v}`")
    md.append("")
    md.append("## 5. Day-2 conclusion")
    md.append(
        "Built `test_top20_soft_support_main.json` by applying the frozen soft-support "
