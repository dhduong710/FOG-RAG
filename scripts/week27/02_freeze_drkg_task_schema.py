...
RAW_DIR = SETTING_DIR / "00_raw_inventory"
TASK_DIR = SETTING_DIR / "01_task_spec"
...
def select_auxiliary_cd_relations(cd_candidates: list[dict[str, Any]], target_relation: str) -> list[dict[str, Any]]:
    ...
    for item in cd_candidates:
        ...
        if source == "Hetionet":
            caution.append("overlaps_with_week26_hetionet_source")
...
def write_report(path: Path, summary: dict[str, Any]) -> None:
    ...
    protocol = summary["protocol_freeze"]

    lines = []
    lines.append("# Week 27 Day 2  DRKG task schema freeze")
    lines.append("")
    ...
    lines.append(f"- Claim level: `{spec['paper_claim_level']}`")
    ...
    lines.append("")
    lines.append("Build the actual coverage-safe split and train graph using this frozen protocol.")
...
def main():
    ...
    task_spec = {
        ...
        "candidate_universe_description": (
            "Candidate compounds are compounds that appear as heads of the frozen target relation in the training split. "
        ...
        "paper_claim_level": "DrugBank treatment-relation prediction within DRKG; not clinical validation.",
    ...
    write_json(protocol_freeze, TASK_DIR / "protocol_freeze.json")
