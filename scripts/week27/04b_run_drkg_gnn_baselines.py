...
ROOT = Path(".")
SPLIT_DIR = ROOT / "dataset" / "setting_e_drkg" / "02_splits"
GRAPH_DIR = ROOT / "dataset" / "setting_e_drkg" / "03_graph"
BASELINE_DIR = ROOT / "dataset" / "setting_e_drkg" / "04_baseline_outputs"
RESULT_DIR = ROOT / "results" / "week27"
REPORT_DIR = ROOT / "reports" / "week27"

VALID_METRICS_PATH = RESULT_DIR / "drkg_baseline_reviewer_safe_valid.json"
TEST_METRICS_PATH = RESULT_DIR / "drkg_baseline_reviewer_safe_test.json"
MAIN_TABLE_PATH = RESULT_DIR / "drkg_baseline_main_table.json"
REPORT_PATH = REPORT_DIR / "day4_drkg_structure_baselines.md"
...
def write_main_table(valid_metrics, test_metrics):
    table = {
        "dataset": "DRKG drugbank_treats benchmark",
        "protocol": {
...
def write_report(table):
    ...
    md = []
    md.append("# Week 26 Day 3  DRKG Structure Baselines")
    md.append("")
    md.append("## Protocol")
    md.append("")
    md.append("- Dataset: DRKG")
    md.append("- Task: `(?, DRUGBANK::treats, disease)`")
    md.append("- Relation normalized: `drugbank_treats`")
    ...
    md.append("## Notes")
    md.append("")
    md.append("- GNN v2 uses target-balanced sampling for sparse DRUGBANK::treats::Compound:Disease training.")
