...
def build_one_source(
    ...
    support_schema = {
        ...
        "note": "DRKG Day5 FOG-RAG-ready package. DistMult is the main source; R-GCN is diagnostic."
    }
    ...
    if not all(a["schema_pass"] for a in audit.values()):
        decision = "DAY5_DRKG_FOGRAG_NEEDS_FIX"
...
def write_report(summary: dict[str, Any]) -> None:
    path = REPORT_DIR / "day5_drkg_fograg_ready.md"
    lines = []
    lines.append("# Week 27 Day 5  DRKG FOG-RAG-ready packages")
    lines.append("")
...
def main():
    ...
    decision = "DAY5_DRKG_FOGRAG_READY"
    if not all(m["decision"] == "DAY5_DRKG_FOGRAG_READY" for m in manifests.values()):
        decision = "DAY5_DRKG_FOGRAG_PARTIAL_OR_NEEDS_FIX"
    ...
    write_json(summary, RESULT_DIR / "day5_drkg_fograg_ready_summary.json")
    write_report(summary)

    print("\n[DONE] Day 5 DRKG FOG-RAG-ready")
    print(json.dumps({
        "decision": decision,
        "main_source": args.main_source,
        "diagnostic_source": args.diagnostic_source,
