...
GRAPH_DIR = SETTING_DIR / "03_graph"
SOURCE_ROOT = SETTING_DIR / "05_backbone_raw_source"
BACKBONE_READY_ROOT = SETTING_DIR / "06_fograg_ready"

SUPPORT_ROOT = SETTING_DIR / "07_support_features"
SOFT_ROOT = SETTING_DIR / "08_soft_support"
E2E_SOFT_ROOT = SETTING_DIR / "09_e2e_soft_support_ready"

RESULT_DIR = ROOT / "results" / "week27"
REPORT_DIR = ROOT / "reports" / "week27"
...
def load_maps(source: str):
    base = BACKBONE_READY_ROOT / source
    entity2id = read_json(base / "entity2id.json")
    id2entity = {int(k): v for k, v in read_json(base / "id2entity.json").items()}
    relation2id = read_json(base / "relation2id.json")
    id2relation = {int(k): v for k, v in read_json(base / "id2relation.json").items()}
    type_map = read_json(base / "type_map.json")
...
def neighbor_set(
    ...
    for r, nb, edge in adj.get(int(node_id), []):
        if int(r) not in relation_ids:
            continue
        if kind is not None and kind_by_id.get(int(nb)) != kind:
            continue
        if int(nb) in seen:
            continue
        seen.add(int(nb))
...
def build_feature_computer(adj, id2relation, kind_by_id, args):
    compound_gene_rel_ids = rel_ids_by_family(id2relation, "compound_gene")
    disease_gene_rel_ids = rel_ids_by_family(id2relation, "disease_gene")
    gene_gene_rel_ids = rel_ids_by_family(id2relation, "gene_gene")
    direct_rel_ids = rel_ids_by_family(id2relation, "target_treats") | rel_ids_by_family(id2relation, "aux_compound_disease")
    ...
    compound_ids = [eid for eid, kind in kind_by_id.items() if kind == "Compound"]
    all_direct_degrees = []
    for cid in compound_ids:
        d = get_direct_degree(cid)
        if d > 0:
    ...
    def compute(candidate_id: int, query_id: int, original_rank: int):
        ...
        # Penalize direct shortcut and overly generic treatment hubs.
        ...
        return {
            ...
            "candidate_direct_degree_target_aux": int(c_direct_degree),
...
def copy_static_files(source: str, out_dir: Path):
    src_base = BACKBONE_READY_ROOT / source
    for name in [
...
def write_report(summary: dict[str, Any], source: str):
    path = REPORT_DIR / f"day6_drkg_soft_support_{source}.md"
    lines = []
    lines.append(f"# Week 27 Day 6  DRKG soft support ({source})")
    lines.append("")
...
def main():
    ...
    parser.add_argument("--source", default="rgcn", choices=["rgcn", "distmult"])
    ...
    parser.add_argument("--disease-gene-cap", type=int, default=240)
    parser.add_argument("--bridge-cap", type=int, default=40)
    parser.add_argument("--shared-norm", type=float, default=2.0)
    parser.add_argument("--bridge-norm", type=float, default=6.0)
    ...
    if args.variant_name is None:
        args.variant_name = f"soft_support_b050_drkg_{source}_main"

    support_dir = mkdir(SUPPORT_ROOT / source)
    soft_dir = mkdir(SOFT_ROOT / source)
    e2e_dir = mkdir(E2E_SOFT_ROOT / source)

    mkdir(RESULT_DIR)
    mkdir(REPORT_DIR)

    entity2id, id2entity, relation2id, id2relation, type_map = load_maps(source)
    ...
    ready_dir = BACKBONE_READY_ROOT / source
    ...
    for split in ["train", "valid", "test"]:
        ...
        backbone_ready_rows = read_json(ready_dir / f"{split}.json")
        ...
        for sr in soft_rows:
            ...
            if base is None:
                raise KeyError(f"Missing ready row split={split} row_index={sr.get('row_index')}")
        ...
        write_json(e2e_rows, e2e_dir / f"{split}.json")
    ...
    summary = {
        ...
        "e2e_audit": e2e_audit,
        "notes": [
            "Soft support re-ranks only existing top-20 candidates.",
            "No valid/test gold injection.",
            "E2E soft-support package preserves source subgraph and changes candidate order/prompt."
        ],
    }

    write_json(summary, RESULT_DIR / f"day6_drkg_soft_support_{source}_summary.json")
    write_json(summary, soft_dir / "soft_support_manifest.json")
    write_json(summary, e2e_dir / "prep_manifest.json")
    write_report(summary, source)

    print("\n[DONE] Day 6 DRKG soft support")
    print(json.dumps({
        "decision": decision,
        "source": source,
        "raw_metrics": raw_metrics,
