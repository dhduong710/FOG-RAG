...
def write_markdown_report(path: Path, inventory: Dict, task_spec: Dict) -> None:
    ...
    lines = []
    lines.append("# Week 26 Day 1 — Hetionet raw inventory\n")
...
def main():
    ...
    report_dir = mkdir(repo / "reports" / "week26")
    result_dir = mkdir(repo / "results" / "week26")
    script_dir = mkdir(repo / "scripts" / "week26")
    ...
    write_json(result_dir / "day1_hetionet_protocol_freeze.json", protocol)
    ...
    print(f"  - {result_dir / 'day1_hetionet_protocol_freeze.json'}")
