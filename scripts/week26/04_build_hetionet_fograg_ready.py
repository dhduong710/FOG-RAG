...
SOURCE_DIR = SETTING_DIR / "05_backbone_raw_source"
READY_DIR = SETTING_DIR / "06_fograg_ready"
...
def write_report(path: Path, summary: dict[str, Any]):
    lines = []
    lines.append("# Week 26 Day 4  Hetionet FOG-RAG-ready package")
    lines.append("")
...
def main():
    ...
    if not all(x["schema_pass"] for x in ready_audit.values()):
        decision = "DAY4_HETIONET_FOGRAG_NEEDS_FIX"
    ...
    write_json(summary, RESULT_DIR / "day4_hetionet_fograg_ready_audit.json")
    write_report(REPORT_DIR / "day4_hetionet_fograg_ready.md", summary)
