...
ROOT = Path(".")
RESULT_ROOT = ROOT / "results" / "week26" / "e2e_hetionet"
REPORT_DIR = ROOT / "reports" / "week26"
SUMMARY_PATH = ROOT / "results" / "week26" / "hetionet_e2e_summary.json"
REPORT_PATH = REPORT_DIR / "day7_hetionet_e2e_closeout.md"

ROWS = {
    "backbone_raw": "dataset/setting_d_hetionet/06_fograg_ready",
    "soft_support_raw": "dataset/setting_d_hetionet/09_e2e_soft_support_ready",
    "fuzzy_retrieval_main": "dataset/setting_d_hetionet/11_e2e_fuzzy_retrieval_ready",
}
...
def match_candidate(pred_raw: Any, candidates: list[str]) -> tuple[str | None, int]:
    ...
    # Fallback: generated text may include extra words after the answer.
    raw_n = norm(pred_raw)
    matches = []
...
def compute_candidate_metrics(dataset_rows: list[dict[str, Any]]) -> dict[str, Any]:
    ...
    mc = top1_counter.most_common(10)
...
def compute_e2e_metrics(pred_rows: list[dict[str, Any]], dataset_rows: list[dict[str, Any]]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
    out_rows = []
    ...
    metrics = {
        ...
        "metric_note": "Paper-facing metric should be the adjusted E2E rank for consistency with prior PrimeKG/PharmKG analyses; strict exact-match is diagnostic.",
    }
...
def collect_one(row_name: str, split: str, dataset_path: Path) -> dict[str, Any]:
    pred_path = RESULT_ROOT / row_name / f"prediction_{split}.json"
    ...
    if len(pred_rows) != len(dataset_rows):
        raise RuntimeError(f"Row count mismatch {row_name} {split}: pred={len(pred_rows)} dataset={len(dataset_rows)}")
...
def write_report(summary: dict[str, Any]) -> None:
    lines = []
    lines.append("# Week 26 Day 7 — Hetionet E2E closeout")
    lines.append("")
    lines.append(f"- Decision: `{summary['decision']}`")
    lines.append("- Model row count: `3`")
    lines.append("- Rows: `backbone_raw`, `soft_support_raw`, `fuzzy_retrieval_main`")
    lines.append("")

    for split in ["valid", "test"]:
        lines.append(f"## {split.capitalize()} metrics")
        lines.append("")
        lines.append("| Row | Cand MRR | E2E adj MRR | E2E H@1 | E2E H@3 | E2E H@10 | Pred-in-cand | Invalid | Avg graph |")
        lines.append("|---|---:|---:|---:|---:|---:|---:|---:|---:|")
        for r in summary["by_split"][split]:
            lines.append(
                f"| {r['row_name']} | {r['candidate_mrr_at20']:.6f} | "
    ...
    lines.append("## Notes")
    lines.append("")
    lines.append("- Candidate metrics should match Day 5/Day 6 values.")
    lines.append("- Fuzzy retrieval should preserve candidate metrics while reducing graph size from 80 to 44.")
    lines.append("- Strict exact-match metrics are saved in JSON files for diagnostics.")
    lines.append("")
...
def main():
    ...
    for split in ["valid", "test"]:
        by_split[split] = sorted(by_split[split], key=lambda x: x["e2e_adjusted_mrr_at20"], reverse=True)
    ...
    decision = "DAY7_HETIONET_E2E_ALL_ROWS_READY" if not missing_rows else "DAY7_HETIONET_E2E_PARTIAL_READY"
    ...
    summary = {
        ...
        "by_split": by_split,
        "metric_note": "Adjusted E2E rank is used for consistency with prior SoftFuse-KGC E2E reporting; strict exact-match metrics are included in per-row JSON outputs.",
    }
