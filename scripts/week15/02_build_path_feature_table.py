...
RAW_PATH = ROOT / "dataset/setting_a/23_noinj_source/valid_top20_raw.json"
EVID_PATH = ROOT / "dataset/setting_a/24b_noinj_evidence/valid_aligned_evidence.json"
SUPPORT_PATH = ROOT / "dataset/setting_a/26_n2_soft_support/valid_top20_soft_support_main.json"
...
OUT_FEATURES = ROOT / "dataset/setting_a/27_n2_fuzzy_retrieval/valid_path_features.json"
OUT_SUMMARY = ROOT / "results/week15/path_feature_summary.json"
OUT_REPORT = ROOT / "reports/week15/day2_path_feature_build.md"
...
def get_valid_b_row_info(row: Dict[str, Any]) -> Dict[str, Any]:
    """
    Cố gắng hỗ trợ nhiều naming style từ tuần trước.
    """
    return {
...
def main() -> None:
    ...
    for i, (raw_row, evid_row, supp_row, vb_row_raw) in enumerate(zip(raw_rows, evid_rows, support_rows, valid_b_rows)):
        ...
        if not isinstance(contra_flags, list) or len(contra_flags) != len(candidate_ids):
            # fallback mềm: nếu valid_b không align, cố lấy contra từ candidate_debug_rows
            fallback = []
        ...
        for triple_index, triple in enumerate(subgraph):
            ...
            for node_id in [head_id, tail_id]:
                if node_id in candidate_info_by_id:
                    ...
                    # direct shortcut = triple nối candidate trực tiếp với query
                    if touches_query:
                        direct_candidate_query_flag = True

            touches_candidate = len(touched_candidate_ids) > 0

            # Chỉ check schema nhất quán nếu biết được tên node
    ...
    print("=" * 80)
    print("DAY 2 PATH FEATURE BUILD DONE")
