...
# Week 25 Day 3: Rule sensitivity E2E inference.
...
SRC_CKPT="${SRC_CKPT:-results/week20/e2e_primary_checkpoint}"
DST_CKPT="results/week25/rule_sensitivity/e2e_checkpoint"

KGE_PATH="dataset/setting_a/08_backbone_ready/entity_embeddings_rgcn.pt"
OUT_DIR="results/week25/rule_sensitivity"
LOG_DIR="${OUT_DIR}/logs"

mkdir -p "${OUT_DIR}" "${LOG_DIR}" "${DST_CKPT}"

echo "================================================================================"
echo "Week25 Day3 Rule Sensitivity E2E"
echo "MODEL_NAME_OR_PATH=${MODEL_NAME_OR_PATH}"
echo "SRC_CKPT=${SRC_CKPT}"
echo "DST_CKPT=${DST_CKPT}"
echo "================================================================================"

# Copy checkpoint into Week25 folder so infer.py writes outputs under results/week25/rule_sensitivity.
...
for variant in "${VARIANTS[@]}"; do
  DATASET_PATH="dataset/setting_a/31_week25_rule_sensitivity/${variant}"
  ...
  for split in "${SPLITS[@]}"; do
    suffix="week25_rule_${variant}"
    log_file="${LOG_DIR}/${split}_${variant}.log"

    echo "================================================================================"
    echo "Running variant=${variant} split=${split}"
    ...
    echo "Done variant=${variant} split=${split}"
  done
done

echo "================================================================================"
echo "All Week25 rule sensitivity E2E runs finished."
