...
"""
Week 25 Day 1: Sensitivity protocol audit and freeze.

This script does NOT run new experiments.
It audits the frozen Week 24 / Week 20 inputs and writes:
- results/week25/protocol/week25_sensitivity_protocol.json
- reports/week25/day1_sensitivity_protocol_audit.md
"""
...
E2E_READY_ROOT = ROOT / "dataset" / "setting_a" / "29_n2_e2e_infer_ready"
...
WEEK24_PATHS = {
    "primekg_e2e_protocol_freeze": ROOT / "results" / "week24" / "primekg_e2e_protocol_freeze.json",
    "week24_go_decision": ROOT / "results" / "week24" / "week24_go_decision.json",
    "frozen_decode_test_dir": ROOT / "results" / "week24" / "frozen_decode_test",
    "model_compare_dir": ROOT / "results" / "week24" / "model_compare",
    "diagnostics_dir": ROOT / "results" / "week24" / "diagnostics",
}

CHECKPOINT_CANDIDATE_ROOTS = [
    ROOT / "results" / "week20" / "e2e_primary_checkpoint" / "checkpoint-final",
    ROOT / "results" / "week20" / "e2e_primary_checkpoint",
    ROOT / "results" / "week24" / "e2e_primary_checkpoint" / "checkpoint-final",
    ROOT / "results" / "week24" / "e2e_primary_checkpoint",
]
...
def find_checkpoint() -> Dict[str, Any]:
    ...
    week20_root = ROOT / "results" / "week20" / "e2e_primary_checkpoint"
    if week20_root.exists():
        candidates.extend(sorted(week20_root.glob("checkpoint-*")))

    week24_root = ROOT / "results" / "week24" / "e2e_primary_checkpoint"
    if week24_root.exists():
        candidates.extend(sorted(week24_root.glob("checkpoint-*")))
...
def audit_week24_paths() -> Dict[str, Any]:
    out = {}
    for name, path in WEEK24_PATHS.items():
        out[name] = {
            "path": rel(path),
            "exists": path.exists(),
            "is_file": path.is_file() if path.exists() else False,
    ...
    return {
        "rule_sensitivity": {
            ...
            "variants": [
                {
                    "name": "main_rules",
                    "description": "Current FOG-RAG retrieval_main with frozen Week 24 rules/support/retrieval behavior.",
...
def collect_fatal_errors(
    ...
    protocol_freeze = week24_audit.get("primekg_e2e_protocol_freeze", {})
    if not protocol_freeze.get("exists"):
        errors.append("Missing results/week24/primekg_e2e_protocol_freeze.json")
...
def collect_warnings(
    ...
    for name, info in week24_audit.items():
        if not info.get("exists"):
            warnings.append(f"Week24 artifact not found: {info.get('path')}")
...
def write_markdown_report(protocol: Dict[str, Any], out_path: Path) -> None:
    ...
    week24_audit = protocol["input_audit"]["week24_artifacts"]
    checkpoint = protocol["input_audit"]["checkpoint"]

    lines: List[str] = []

    lines.append("# Week 25 Day 1  Sensitivity Protocol Audit\n")
    ...
    lines.append(f"- Frozen decoding config: **{protocol['frozen_decoding']['config_name']}**")
    lines.append("")

    lines.append("## Frozen decoding\n")
    lines.append("```json")
    lines.append(json.dumps(protocol["frozen_decoding"], indent=2, ensure_ascii=False))
    lines.append("```\n")

    lines.append("## Week 24 reference table\n")
    ref_rows = []
    ref = protocol["week24_reference"]["primekg_locked_test_llama3_2_3b"]
    ...
    lines.append("## Week 24 artifact audit\n")
    artifact_rows = []
    for name, info in week24_audit.items():
    ...
    lines.append("## Sensitivity groups frozen for Week 25\n")
    ...
    lines.append(
        "Week 25 sensitivity outputs are appendix/supporting evidence only. "
        "They must not replace the Week 24 locked main result, and no test-based tuning is allowed."
    )
    lines.append("")
...
def main() -> None:
    ...
    week24_audit = audit_week24_paths()

    fatal_errors = collect_fatal_errors(e2e_audit, checkpoint_audit, week24_audit)
    warnings = collect_warnings(e2e_audit, week24_audit)

    decision = (
        "WEEK25_SENSITIVITY_PROTOCOL_READY"
        if len(fatal_errors) == 0
        else "WEEK25_SENSITIVITY_PROTOCOL_BLOCKED"
    )

    protocol = {
        "week": 25,
        "day": 1,
        "title": "Sensitivity protocol audit and freeze",
        ...
        "metric_policy": {
            ...
            "raw_infer_py_mrr": "audit_only_not_paper_metric",
            "gold_injection": False,
            "test_tuning": False,
        },
        "week24_reference": WEEK24_REFERENCE,
        "sensitivity_groups": build_sensitivity_groups(),
        "input_audit": {
            "e2e_rows": e2e_audit,
            "checkpoint": checkpoint_audit,
            "week24_artifacts": week24_audit,
        },
        "fatal_errors": fatal_errors,
        "warnings": warnings,
        "do_not_change": [
            "Do not overwrite results/week24 artifacts.",
            "Do not change dataset/setting_a/29_n2_e2e_infer_ready source packages.",
            "Do not tune decoding/template/rules on test.",
            "Do not promote Week 25 sensitivity variants as a new contribution.",
            "Do not use raw infer.py MRR as the paper metric.",
        ],
        "outputs": {
            "protocol_json": "results/week25/protocol/week25_sensitivity_protocol.json",
            "audit_report_md": "reports/week25/day1_sensitivity_protocol_audit.md",
        },
    }

    protocol_path = PROTOCOL_DIR / "week25_sensitivity_protocol.json"
    report_path = REPORTS_DIR / "day1_sensitivity_protocol_audit.md"
    ...
    print(f"protocol_json = {rel(protocol_path)}")
