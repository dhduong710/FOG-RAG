...
"""
Week 25 Day 5: Build small-noise robustness variants.
...
SRC_MAIN = ROOT / "dataset" / "setting_a" / "29_n2_e2e_infer_ready" / "retrieval_main"
OUT_ROOT = ROOT / "dataset" / "setting_a" / "33_week25_noise_robustness"
RESULTS_DIR = ROOT / "results" / "week25" / "noise_robustness"
...
def add_support_score_noise(row: Dict[str, Any], std: float, seed: int, row_idx: int, split: str) -> Dict[str, Any]:
    ...
    if not isinstance(scores, list) or len(scores) != len(r.get("rank_entities", [])):
        r["week25_noise_warning"] = "missing_or_misaligned_support_scores"
        return r
    ...
    r["week25_noise_kind"] = "support_score_noise"
...
def add_subgraph_dropout(row: Dict[str, Any], dropout_rate: float, seed: int, row_idx: int, split: str) -> Dict[str, Any]:
    ...
    r["week25_noise_kind"] = "subgraph_edge_dropout"
...
def annotate(row: Dict[str, Any], variant: str, split: str, cfg: Dict[str, Any]) -> Dict[str, Any]:
    r = copy.deepcopy(row)
    r["week25_noise_variant"] = variant
    r["week25_noise_config"] = cfg
    r["week25_split"] = split
    r["infer_row_name"] = f"week25_noise_{variant}"
    return r
...
def main() -> None:
    if not PROTOCOL_PATH.exists():
        raise FileNotFoundError(f"Missing Day 1 protocol: {rel(PROTOCOL_PATH)}")

    protocol = load_json(PROTOCOL_PATH)
    if protocol.get("decision") != "WEEK25_SENSITIVITY_PROTOCOL_READY":
        raise RuntimeError(f"Day 1 protocol is not READY: {protocol.get('decision')}")
