...
"""
Week 25 Day 2: Build rule sensitivity variants.
...
PROTOCOL_PATH = ROOT / "results" / "week25" / "protocol" / "week25_sensitivity_protocol.json"

SRC_ROOT = ROOT / "dataset" / "setting_a" / "29_n2_e2e_infer_ready"
SRC_MAIN = SRC_ROOT / "retrieval_main"
SRC_SOFT = SRC_ROOT / "soft_support_raw"

OUT_ROOT = ROOT / "dataset" / "setting_a" / "31_week25_rule_sensitivity"
RESULTS_DIR = ROOT / "results" / "week25" / "rule_sensitivity"
...
def annotate_row(row: Dict[str, Any], variant: str, split: str, source_desc: str) -> Dict[str, Any]:
    ...
    out["week25_rule_variant"] = variant
    out["week25_rule_variant_source"] = source_desc
    out["week25_split"] = split
    out["infer_row_name"] = f"week25_rule_{variant}"
    return out
...
def build_main_rules(split: str, main_rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for row in main_rows:
        r = annotate_row(
            ...
            variant="main_rules",
            split=split,
            source_desc="exact_copy_of_dataset/setting_a/29_n2_e2e_infer_ready/retrieval_main",
        )
...
def build_no_rules(split: str, soft_rows: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for row in soft_rows:
        r = annotate_row(
            ...
            split=split,
            source_desc=(
                "soft_support_raw_package_without_week15_16_confidence_aware_fuzzy_rule_selection"
            ),
        )
        source_subgraph = extract_subgraph(row)
        r["subgraph"] = source_subgraph
        r["selected_subgraph"] = source_subgraph
        r["selected_source_variant"] = "week25_no_rules_soft_support_raw_subgraph"
        r["subgraph_summary"] = {
            "variant": "no_rules",
...
def build_random_rules(
    ...
    for i, (main_row, soft_row) in enumerate(zip(main_rows, soft_rows)):
        ...
        r["selected_source_variant"] = "week25_random_rules_negative_control"
        r["triple_score_rows"] = []
        r["path_scores"] = []
        r["subgraph_summary"] = {
            "variant": "random_rules",
...
def main() -> None:
    if not PROTOCOL_PATH.exists():
        raise FileNotFoundError(
            f"Missing Day 1 protocol: {rel(PROTOCOL_PATH)}. Run Day 1 first."
        )

    protocol = load_json(PROTOCOL_PATH)
    if protocol.get("decision") != "WEEK25_SENSITIVITY_PROTOCOL_READY":
        raise RuntimeError(
            f"Day 1 protocol is not READY: {protocol.get('decision')}"
        )
    ...
    for variant in VARIANTS:
        v_manifest = {
            ...
            "may_replace_week24_main_result": False,
        }
