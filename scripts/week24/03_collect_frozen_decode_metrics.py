...
PRED_ROOT = ROOT / "results/week24/frozen_decode_test/predictions"
READY_ROOT = ROOT / "dataset/setting_a/29_n2_e2e_infer_ready"

OUT_VALID = ROOT / "results/week24/frozen_decode_test/primekg_e2e_final_table_valid.json"
OUT_TEST = ROOT / "results/week24/frozen_decode_test/primekg_e2e_final_table_test.json"
OUT_COMPARISON = ROOT / "results/week24/frozen_decode_test/week20_vs_week24_e2e_comparison.json"
OUT_SUMMARY = ROOT / "results/week24/frozen_decode_test/primekg_frozen_decode_summary.json"
OUT_MD = ROOT / "reports/week24/day3_frozen_decode_test_rerun.md"

BEST_CONFIG = ROOT / "results/week24/frozen_decode_test/frozen_decode_config.json"
...
def main() -> None:
    ...
    week24_test = {r["row_name"]: r for r in by_split["test"]}
    comparison = {
        "decision": "WEEK20_VS_WEEK24_COMPARISON_BUILT",
        "week20_reference": WEEK20_TEST_REFERENCE,
        "week24_test": by_split["test"],
        "delta_week24_minus_week20": {},
    }

    for row_name, old in WEEK20_TEST_REFERENCE.items():
        new = week24_test[row_name]
        comparison["delta_week24_minus_week20"][row_name] = {
    ...
    checks = {
        "soft_improves_backbone_test_e2e": (
            week24_test["soft_support_raw"]["reviewer_safe_e2e_mrr_at20"]
            > week24_test["backbone_raw"]["reviewer_safe_e2e_mrr_at20"]
        ),
        "retrieval_preserves_or_improves_soft_test_e2e": (
            week24_test["retrieval_main"]["reviewer_safe_e2e_mrr_at20"]
            >= week24_test["soft_support_raw"]["reviewer_safe_e2e_mrr_at20"]
        ),
        "retrieval_smaller_than_soft": (
            week24_test["retrieval_main"]["avg_subgraph_size"]
            < week24_test["soft_support_raw"]["avg_subgraph_size"]
        ),
        "retrieval_candidate_coverage_preserved": (
            week24_test["retrieval_main"].get("avg_candidate_coverage_preserved_rate") == 1.0
        ),
    }
    ...
    summary = {
        ...
        "week20_vs_week24_comparison": comparison,
    }

    save_json(OUT_SUMMARY, summary)

    lines = []
    lines.append("# Week 24 Day 3 — Frozen Decoding Valid/Test Rerun")
    lines.append("")
    ...
    for row_name, old in WEEK20_TEST_REFERENCE.items():
        new = week24_test[row_name]
        delta = comparison["delta_week24_minus_week20"][row_name]["delta_e2e_mrr_at20"]
        lines.append(
