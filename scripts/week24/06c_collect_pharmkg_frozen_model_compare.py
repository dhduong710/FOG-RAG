...
MODEL_ROOT = ROOT / "results/week24/pharmkg_frozen_model_compare"
READY_ROOT = ROOT / "dataset/setting_c_pharmkg/13_e2e_infer_ready"
...
ASSET_DIR = MODEL_ROOT / "paper_assets"
...
OUT_MD = ROOT / "reports/week24/day6c_pharmkg_frozen_model_compare.md"
...
def latex_table_e2e(primary_rows: List[Dict[str, Any]]) -> str:
    names = {
        ...
        "fuzzy_retrieval_main": "FOG-RAG retrieval",
    }
    lines = [
        r"\begin{table}[t]",
        r"\centering",
...
def latex_table_model(test_fuzzy: List[Dict[str, Any]]) -> str:
    ...
    lines += [
        r"\hline",
        r"\end{tabular}",
        r"\caption{PharmKG base-LLM comparison on the FOG-RAG retrieval row.}",
...
def main() -> None:
    ...
    limit_para = (
        ...
        "FOG-RAG improvements, not as a full-universe PharmKG KGC superiority claim."
    )
    ...
    lines = []
    lines.append("# Week 24 Extension Day 6C  PharmKG Frozen Model Comparison")
    lines.append("")
