...
# Important: this directory itself must contain adapter_config.json, adapter_model.*, graph_model.bin.
# If your checkpoint is under checkpoint-final, override CKPT when running.
CKPT="${CKPT:-results/week20/e2e_primary_checkpoint}"

CONFIG_JSON="results/week24/decoding_sweep_valid/configs.json"
OUT_ROOT="results/week24/decoding_sweep_valid"
PRED_OUT="${OUT_ROOT}/decoding_sweep_predictions"

mkdir -p "${OUT_ROOT}" "${PRED_OUT}" "reports/week24"
...
python - <<'PY' > results/week24/decoding_sweep_valid/config_lines.tsv
import json
p = "results/week24/decoding_sweep_valid/configs.json"
...
done < results/week24/decoding_sweep_valid/config_lines.tsv
