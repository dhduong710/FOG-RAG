...
# User config
# ============================================================
ROOT="dataset/setting_a/29_n2_e2e_infer_ready"
KGE="dataset/setting_a/08_backbone_ready/entity_embeddings_rgcn.pt"
OUT_ROOT="results/week24/model_compare"
LOG_ROOT="reports/week24/model_compare"

BEST_JSON="results/week24/decoding_sweep_valid/decoding_sweep_best_config.json"

# Llama-3.2-3B checkpoint from Week20/Day3.
# This path should directly contain adapter_config.json, adapter_model.*, graph_model.bin.
LLAMA32_CKPT="${LLAMA32_CKPT:-results/week20/e2e_primary_checkpoint}"
...
# Set RETRAIN_LLAMA32=1 if you want to train it again under results/week24/model_compare.
...
# Read frozen decoding config from Day 2.
read MAX_NEW REP_PEN NGRAM CONFIG_ID < <(python - <<'PY'
import json
p = "results/week24/decoding_sweep_valid/decoding_sweep_best_config.json"
...
echo "============================================================"
echo "Week 24 Day 4 — PrimeKG model comparison train/infer"
echo "============================================================"
echo "ROOT=${ROOT}"
echo "KGE=${KGE}"
  ...
  cat > "${MODEL_DIR}/model_run_manifest.json" <<EOF
{
  ...
  "paper_metric_note": "Raw infer.py metrics are audit-only. Day 5 recomputes reviewer-safe metrics."
}
EOF
