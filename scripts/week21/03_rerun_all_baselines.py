...
- outputs are top-20 candidate rows for Day 4 reviewer-safe metric recomputation
"""
...
ROOT = Path(".")
OUT_ROOT = ROOT / "results" / "week21" / "baseline_outputs"
REPORT_DIR = ROOT / "reports" / "week21"
REPORT_PATH = REPORT_DIR / "day3_rerun_or_recollect_structure_baselines.md"

VALID_QUERY_PATH = ROOT / "dataset" / "setting_a" / "23_noinj_source" / "valid_top20_raw.json"
TEST_QUERY_PATH = ROOT / "dataset" / "setting_a" / "23_noinj_source" / "test_top20_raw.json"

TRAIN_ROW_CANDIDATES = [
    ROOT / "dataset" / "setting_a" / "29_n2_e2e_infer_ready" / "backbone_raw" / "train.json",
    ROOT / "dataset" / "setting_a" / "24b_noinj_evidence" / "train_aligned_evidence.json",
    ROOT / "dataset" / "setting_a" / "08_backbone_ready" / "train.json",
    ROOT / "dataset" / "setting_a" / "12_backbone_ready_ranker_v2" / "train.json",
    ROOT / "archive_pre_n2" / "dataset" / "setting_a" / "12_backbone_ready_ranker_v2" / "train.json",
    ROOT / "archive_pre_n2" / "dataset" / "setting_a" / "08_backbone_ready" / "train.json",
]
...
def find_enriched_kg() -> Path | None:
    ...
    roots = [ROOT / "dataset", ROOT / "archive_pre_n2", ROOT / "results"]
...
def maybe_parse_type_map_for_drugs(id2name: dict[int, str]) -> set[int]:
    ...
    paths = []
    for root in [ROOT / "dataset", ROOT / "archive_pre_n2", ROOT / "results"]:
        if root.exists():
...
def write_day3_report(all_summaries: list[dict[str, Any]], data: DataPack, args: argparse.Namespace) -> None:
    REPORT_DIR.mkdir(parents=True, exist_ok=True)

    lines = []
    lines.append("# Week 21 Day 3 — Rerun All Structure Baselines\n")
    ...
    lines.append("- Reviewer-safe metrics will be recomputed on Day 4.\n")
    ...
    for s in all_summaries:
        ...
        lines.append(
            ...
            f"| `results/week21/baseline_outputs/{m}` |"
        )
    ...
    for s in all_summaries:
        m = s["model_name"]
        lines.append(f"### {m}")
        lines.append(f"- `results/week21/baseline_outputs/{m}/valid_top20.json`")
        lines.append(f"- `results/week21/baseline_outputs/{m}/test_top20.json`")
        lines.append(f"- `results/week21/baseline_outputs/{m}/config.json`")
        lines.append(f"- `results/week21/baseline_outputs/{m}/train_log.json`")
        lines.append(f"- `results/week21/baseline_outputs/{m}/summary.json`")

    lines.append("\n## Next step\n")
    lines.append(
        "Day 4 should recompute reviewer-safe metrics for all baseline outputs and FOG-RAG rows "
        "with the frozen rule: RR = 1/rank if rank <= 20 else 0; absent gold has rank 21 and RR 0."
    )
...
def main() -> None:
    ...
    print("=" * 100)
    print("WEEK 21 DAY 3 — RERUN ALL BASELINES")
    print("=" * 100)
    print("device:", device)
    ...
    (ROOT / "results" / "week21" / "baseline_rerun_summary.json").write_text(
        json.dumps(all_summaries, indent=2, ensure_ascii=False) + "\n",
        encoding="utf-8",
    )
    write_day3_report(all_summaries, data, args)

    print("=" * 100)
    print("DAY 3 DONE")
    print("Summary:", ROOT / "results" / "week21" / "baseline_rerun_summary.json")
