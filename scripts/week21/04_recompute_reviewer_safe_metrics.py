...
"""
Week 21 Day 4
Recompute reviewer-safe metrics for rerun baselines and FOG-RAG rows.
...
OUT_FOGRAG_VALID = RESULTS_DIR / "fograg_reviewer_safe_valid.json"
OUT_FOGRAG_TEST = RESULTS_DIR / "fograg_reviewer_safe_test.json"
OUT_ALL = RESULTS_DIR / "day4_reviewer_safe_metrics_all.json"
OUT_MD = REPORTS_DIR / "day4_reviewer_safe_metric_recompute.md"
...
FOGRAG_PATHS = {
    "valid": {
        ...
        "soft_support_fuzzy_retrieval_main": ROOT / "dataset" / "setting_b" / "07_n2_eval_valid" / "valid_retrieval_main_eval.json",
    },
    "test": {
        ...
        "soft_support_fuzzy_retrieval_main": ROOT / "dataset" / "setting_b" / "08_n2_eval_test" / "test_retrieval_main_eval.json",
    },
}

RAW_REFERENCE_PATHS = {
    "valid": ROOT / "dataset" / "setting_a" / "23_noinj_source" / "valid_top20_raw.json",
    "test": ROOT / "dataset" / "setting_a" / "23_noinj_source" / "test_top20_raw.json",
}
...
def load_fograg_metrics(split: str) -> list[dict[str, Any]]:
    out = []
    for row_name, path in FOGRAG_PATHS[split].items():
        rows = load_rows(path)
...
def write_report(all_obj: dict[str, Any]) -> None:
    ...
    md = f"""# Week 21 Day 4 — Reviewer-safe Metric Recompute
...
def main() -> None:
    ...
    fograg_valid = load_fograg_metrics("valid")
    fograg_test = load_fograg_metrics("test")

    write_json(OUT_BASELINE_VALID, baseline_valid)
    write_json(OUT_BASELINE_TEST, baseline_test)
    write_json(OUT_FOGRAG_VALID, fograg_valid)
    write_json(OUT_FOGRAG_TEST, fograg_test)
    ...
    all_obj = {
        ...
        "valid": {
            ...
            "fograg_rows": fograg_valid,
            "all_rows": baseline_valid + fograg_valid,
            "all_rows_sorted": sort_rows(baseline_valid + fograg_valid),
        },
        "test": {
            "baseline_rows": baseline_test,
            "fograg_rows": fograg_test,
            "all_rows": baseline_test + fograg_test,
            "all_rows_sorted": sort_rows(baseline_test + fograg_test),
        },
    }
    ...
    print("=" * 100)
    print("WEEK 21 DAY 4 — REVIEWER-SAFE METRIC RECOMPUTE")
    print("=" * 100)
    print(f"Wrote: {OUT_BASELINE_VALID}")
    print(f"Wrote: {OUT_BASELINE_TEST}")
    print(f"Wrote: {OUT_FOGRAG_VALID}")
    print(f"Wrote: {OUT_FOGRAG_TEST}")
    ...
    print("Decision: DAY4_REVIEWER_SAFE_METRICS_RECOMPUTED_READY_FOR_MAIN_TABLE")
    print("=" * 100)
