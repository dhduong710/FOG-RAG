...
"""
Week 22 Day 6: Build PharmKG Dataset 2 FOG-RAG-ready package.

Input:
    dataset/setting_c_pharmkg/02_splits/{train,valid,test}.json
    dataset/setting_c_pharmkg/02_splits/candidate_universe.json
    dataset/setting_c_pharmkg/03_graph/{entity2id,id2entity,relation2id,id2relation,type_map,train_enriched.tsv}
    dataset/setting_c_pharmkg/04_baseline_outputs/rgcn/{valid,test}_top20.json
    results/week22/dataset2_baseline_main_table.json

Output:
    dataset/setting_c_pharmkg/05_backbone_raw_source/
    dataset/setting_c_pharmkg/06_fograg_ready/
    results/week22/dataset2_source_selection.json
    reports/week22/day6_fograg_ready_package.md
"""
...
SPLIT_DIR = ROOT / "dataset" / "setting_c_pharmkg" / "02_splits"
GRAPH_DIR = ROOT / "dataset" / "setting_c_pharmkg" / "03_graph"
BASELINE_DIR = ROOT / "dataset" / "setting_c_pharmkg" / "04_baseline_outputs"

RAW_SOURCE_DIR = ROOT / "dataset" / "setting_c_pharmkg" / "05_backbone_raw_source"
FOGRAG_READY_DIR = ROOT / "dataset" / "setting_c_pharmkg" / "06_fograg_ready"
...
REPORT_PATH = REPORT_DIR / "day6_fograg_ready_package.md"
...
def ensure_dirs() -> None:
    for path in [RAW_SOURCE_DIR, FOGRAG_READY_DIR, RESULT_DIR, REPORT_DIR]:
...
def write_prompt_lexicon(relation2id: dict[str, int]) -> dict[str, Any]:
    ...
    write_json(lexicon, FOGRAG_READY_DIR / "prompt_lexicon.json")
    return lexicon
...
def write_rules_and_schema(relation2id: dict[str, int], id2relation: dict[int, str]) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    write_json(rules, FOGRAG_READY_DIR / "rules.json")
    write_json(support_schema, FOGRAG_READY_DIR / "support_schema.json")
...
def copy_maps_as_pickle(
    ...
    write_pickle(entity2id, FOGRAG_READY_DIR / "entity2id.pkl")
    write_pickle(id2entity, FOGRAG_READY_DIR / "id2entity.pkl")
    write_pickle(relation2id, FOGRAG_READY_DIR / "relation2id.pkl")
    write_pickle(id2relation, FOGRAG_READY_DIR / "id2relation.pkl")

    write_json(entity2id, FOGRAG_READY_DIR / "entity2id.json")
    write_json({str(k): v for k, v in id2entity.items()}, FOGRAG_READY_DIR / "id2entity.json")
    write_json(relation2id, FOGRAG_READY_DIR / "relation2id.json")
    write_json({str(k): v for k, v in id2relation.items()}, FOGRAG_READY_DIR / "id2relation.json")
...
def choose_source_model(metrics: dict[str, Any], requested_source: str) -> dict[str, Any]:
    ...
    selection = {
        ...
        "main_fograg_source": requested_source,
        "main_fograg_source_reason": (
            "R-GCN is selected as the main FOG-RAG transfer source because it is "
            "DrKGC-compatible, aligned with the PrimeKG FOG-RAG pipeline, and it "
        ...
        "note": (
            ...
            "FOG-RAG source for pipeline continuity."
        ),
    }
...
def write_report(
    ...
    md = f"""# Week 22 Day 6 — Build PharmKG FOG-RAG-ready Package
...
- Main FOG-RAG source: `{source_selection["main_fograg_source"]}`
- DrKGC-aligned alternative: `{source_selection["drkgc_aligned_alternative"]}`

Reason:

{source_selection["main_fograg_source_reason"]}
...
def main() -> None:
    ...
    ready_train = raw_source_to_fograg_rows(
    ...
    ready_valid = raw_source_to_fograg_rows(
    ...
    ready_test = raw_source_to_fograg_rows(
    ...
    write_json(ready_train, FOGRAG_READY_DIR / "train.json")
    write_json(ready_valid, FOGRAG_READY_DIR / "valid.json")
    write_json(ready_test, FOGRAG_READY_DIR / "test.json")
    ...
    manifest = {
        "week": 22,
        "day": 6,
        "decision": "FOGRAG_READY_PACKAGE_BUILT" if leak_sanity["decision"] == "PASS" else "FOGRAG_READY_PACKAGE_LEAK_CHECK_FAILED",
        "dataset": "PharmKG-8k",
        "setting": "setting_c_pharmkg",
        "task": {
        ...
        "files": {
            ...
            "fograg_ready_dir": str(FOGRAG_READY_DIR),
            "train_json": str(FOGRAG_READY_DIR / "train.json"),
            "valid_json": str(FOGRAG_READY_DIR / "valid.json"),
            "test_json": str(FOGRAG_READY_DIR / "test.json"),
            "prep_manifest": str(FOGRAG_READY_DIR / "prep_manifest.json"),
        },
    }

    write_json(source_selection, RAW_SOURCE_DIR / "source_selection.json")
    write_json(source_selection, SOURCE_SELECTION_RESULT_PATH)
    write_json(manifest, FOGRAG_READY_DIR / "prep_manifest.json")
    ...
    print(f"  {FOGRAG_READY_DIR / 'train.json'}")
    print(f"  {FOGRAG_READY_DIR / 'valid.json'}")
    print(f"  {FOGRAG_READY_DIR / 'test.json'}")
    print(f"  {FOGRAG_READY_DIR / 'prompt_lexicon.json'}")
    print(f"  {FOGRAG_READY_DIR / 'rules.json'}")
    print(f"  {FOGRAG_READY_DIR / 'support_schema.json'}")
    print(f"  {FOGRAG_READY_DIR / 'prep_manifest.json'}")
    ...
    if manifest["decision"] != "FOGRAG_READY_PACKAGE_BUILT":
        raise RuntimeError(
