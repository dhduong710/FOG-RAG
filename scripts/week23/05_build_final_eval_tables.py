...
Also compares FOG-RAG rows against six Week 22 structure baselines:
- transe
- distmult
- complex
- rotate
...
TRANSFER_EVAL_DIR = DATASET_DIR / "07_transfer_eval_ready"
...
FOGRAG_VALID_TABLE_PATH = RESULT_DIR / "dataset2_fograg_main_table_valid.json"
FOGRAG_TEST_TABLE_PATH = RESULT_DIR / "dataset2_fograg_main_table_test.json"
...
def display_name(row_name: str) -> str:
    return {
        ...
        "fuzzy_retrieval_main": "FOG-RAG main / fuzzy retrieval",
        "transe": "TransE",
        "distmult": "DistMult",
        "complex": "ComplEx",
        "rotate": "RotatE",
...
def build_fograg_table(split: str) -> list[dict[str, Any]]:
    rows = []
    for row_name in FOGRAG_ROWS:
...
def build_claim_summary(
    ...
    decision = "DATASET2_FOGRAG_MAIN_TABLE_READY"
    ...
    if (
        ...
        paper_status = "GO_PAPER_DATASET2_TRANSFER_RESULT"
    elif (
        fuzzy_valid["mrr_at20"] > backbone_valid["mrr_at20"]
        or fuzzy_test["mrr_at20"] > backbone_test["mrr_at20"]
    ):
        paper_status = "PARTIAL_TRANSFER_DIAGNOSTIC_OR_APPENDIX"
    else:
        paper_status = "APPENDIX_ONLY_OR_DIAGNOSTIC"

    return {
        "week": 23,
        "day": 5,
        "decision": decision,
        "paper_status_candidate": paper_status,
        ...
        "main_findings": {
            ...
            "vs_structure_baselines": {
                ...
                "interpretation": "FOG-RAG main has the best MRR@20 under the task-specific reviewer-safe protocol.",
            },
        },
        "safe_claim": (
            "On the PharmKG therapeutic-association proxy benchmark, frozen soft support improves "
...
def main() -> None:
    ...
    valid_table = build_fograg_table("valid")
    test_table = build_fograg_table("test")
    ...
    write_json(valid_table, FOGRAG_VALID_TABLE_PATH)
    write_json(test_table, FOGRAG_TEST_TABLE_PATH)
    ...
    print(f"  {FOGRAG_VALID_TABLE_PATH}")
    print(f"  {FOGRAG_TEST_TABLE_PATH}")
    print(f"  {VS_BASELINES_PATH}")
    print(f"  {CLAIM_SUMMARY_PATH}")
    print(f"  {REPORT_PATH}")

    print("\nDecision:", claim["decision"])
    print("Paper status candidate:", claim["paper_status_candidate"])
