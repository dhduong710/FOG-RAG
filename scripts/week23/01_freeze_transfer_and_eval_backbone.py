...
"""
Week 23 Day 1: Freeze PharmKG transfer protocol and evaluate backbone_raw.
...
EVAL_READY_DIR = ROOT / "dataset" / "setting_c_pharmkg" / "07_transfer_eval_ready"

RESULT_WEEK22_DIR = ROOT / "results" / "week22"
...
PROTOCOL_PATH = RESULT_DIR / "dataset2_transfer_protocol_freeze.json"
...
def build_protocol() -> dict[str, Any]:
    return {
        "week": 23,
        "day": 1,
        "decision": "TRANSFER_PROTOCOL_FROZEN",
        "dataset": "PharmKG-8k",
        ...
        "task": {
            ...
            "paper_label": "therapeutic association proxy",
            "do_not_call_it": [
                "clinical indication",
                "PrimeKG indication",
        ...
        "week23_rows_to_build": [
            "backbone_raw",
            "hard_support_raw",
            "soft_support_raw",
            "fuzzy_retrieval_main",
        ],
        "day1_expected_backbone_metrics_from_week22": {
            "valid": {
...
def find_week22_rgcn_metric(split: str) -> dict[str, Any] | None:
...
def compare_with_week22(metric: dict[str, Any], split: str) -> dict[str, Any]:
    ref = find_week22_rgcn_metric(split)
    if ref is None:
        return {
            "available": False,
            "match": None,
    ...
    return {
        "available": True,
        "match": bool(match),
        "week22_rgcn_metric": ref,
        "week23_backbone_metric": metric,
        "diffs": diffs,
    }
...
def write_report(
    ...
    md = f"""# Week 23 Day 1 — Transfer Protocol Freeze and Backbone Raw Evaluation
...
- Paper label: `{protocol["task"]["paper_label"]}`
...
- valid match Week 22 R-GCN: `{valid_compare["match"]}`
- test match Week 22 R-GCN: `{test_compare["match"]}`
...
def main() -> None:
    ...
    ready_valid = read_json(FOGRAG_READY_DIR / "valid.json")
    ready_test = read_json(FOGRAG_READY_DIR / "test.json")
    ...
    valid_compare = compare_with_week22(valid_metrics, "valid")
    test_compare = compare_with_week22(test_metrics, "test")
    ...
    protocol["week22_consistency_check"] = {
        "valid": valid_compare,
        "test": test_compare,
    }
    ...
    if valid_compare["available"] and not valid_compare["match"]:
        raise RuntimeError("Valid metrics do not match Week 22 R-GCN baseline.")
    if test_compare["available"] and not test_compare["match"]:
        raise RuntimeError("Test metrics do not match Week 22 R-GCN baseline.")
