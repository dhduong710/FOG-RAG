...
INPUTS = {
    ...
    "encoder_compare_week17": ROOT / "results/week17/encoder_probe_compare.json",
    "encoder_probe_rows": ROOT / "dataset/setting_a/28_n2_fuzzy_encoder/valid_fuzzy_encoder_probe_v0.json",
}

OUT_TABLE = ROOT / "results/week18/novelty2_valid_main_table.json"
OUT_ABLATION = ROOT / "results/week18/novelty2_valid_ablation.json"
OUT_REPORT = ROOT / "reports/week18/day3_valid_main_table.md"
...
def summarize_encoder_probe_reviewer_safe(probe_rows: list[dict]) -> dict:
    ranks = [int(r["probe_summary"]["gold_rank_probe"]) for r in probe_rows]
    n = len(ranks)
    num_present = sum(1 for r in ranks if r <= 20)

    rr_at20_items = [(1.0 / r) if r <= 20 else 0.0 for r in ranks]
    rr_present_items = [(1.0 / r) for r in ranks if r <= 20]

    avg_bridge_norm = safe_mean([float(r["probe_summary"]["avg_bridge_norm"]) for r in probe_rows])
    ...
    return {
        ...
        "hits10_at20": round(sum(1 for r in ranks if r <= 10) / n, 6),
        "avg_gold_rank": round(sum(ranks) / n, 6),
        "avg_bridge_norm": round(avg_bridge_norm, 6),
    }


def build_ablation(table_rows: list[dict], encoder_compare: dict, encoder_probe_rows: list[dict]) -> dict:
    ...
    probe_vs_retrieval = encoder_compare["probe_vs_retrieval_main"]
    encoder_probe = summarize_encoder_probe_reviewer_safe(encoder_probe_rows)
    ...
    appendix_probe_summary = {
        "row_name": "soft_support_fuzzy_encoder_probe_v0",
        "positioning": "supporting_appendix_only",
        "gold_present_rate": encoder_probe["gold_present_rate"],
        "mrr_at20": encoder_probe["mrr_at20"],
        "mrr_present_only": encoder_probe["mrr_present_only"],
        "hits1_at20": encoder_probe["hits1_at20"],
        "avg_gold_rank": encoder_probe["avg_gold_rank"],
        "avg_bridge_norm": encoder_probe["avg_bridge_norm"],
        "improved_vs_retrieval_main": probe_vs_retrieval["improved_vs_retrieval_main"],
        "worsened_vs_retrieval_main": probe_vs_retrieval["worsened_vs_retrieval_main"],
        "avg_rank_delta_probe_minus_retrieval": probe_vs_retrieval["avg_rank_delta_probe_minus_retrieval"],
    ...
    return {
        ...
        "appendix_probe_summary": appendix_probe_summary,
        "key_takeaways": [
...
def main():
    ...
    encoder_compare = load_json(INPUTS["encoder_compare_week17"])
    encoder_probe_rows = load_json(INPUTS["encoder_probe_rows"])
    ablation = build_ablation(table_rows, encoder_compare, encoder_probe_rows)
    ...
    lines = []
    lines.append("# Week 18 Day 3 — Valid Main Table + Ablation (Reviewer-safe)\n")
    ...
    for item in ablation["key_takeaways"]:
        lines.append(f"- {item}")
    lines.append("")
    lines.append("## Appendix note")
    aps = ablation["appendix_probe_summary"]
    lines.append(f"- encoder probe positioning: `{aps['positioning']}`")
    lines.append(f"- encoder probe decision: `{aps['decision']}`")
    lines.append("")
