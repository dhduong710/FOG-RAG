...
DAY3_SUMMARY = ROOT / "results/week20/e2e_backbone_soft_summary.json"

RETR_RAW_METRICS = ROOT / "results/week20/ranking_metrics_test_retrieval_main_e2e.json"
RETR_PRED = ROOT / "results/week20/prediction_test_retrieval_main_e2e.json"
RETR_INFER_READY = ROOT / "dataset/setting_a/29_n2_e2e_infer_ready/retrieval_main/test.json"

OUT_JSON = ROOT / "results/week20/e2e_retrieval_summary.json"
OUT_MD = ROOT / "reports/week20/day4_run_retrieval_main.md"
...
def main() -> None:
    ...
    summary = {
        ...
        "metric_policy": {
            ...
            "important_note": (
                ...
                "Paper-facing metrics use reviewer-safe recomputation from prediction rows."
            ),
        },
        "retrieval_main": {
    ...
    md = []
    md.append("# Day 4 — E2E infer for retrieval_main")
    md.append("")
    ...
    md.append(
        "Raw `infer.py` metrics are retained only for audit. "
        "Paper-facing metrics should use the reviewer-safe recomputation in this report."
    )
