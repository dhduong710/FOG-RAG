...
DAY3 = ROOT / "results/week20/e2e_backbone_soft_summary.json"
DAY4 = ROOT / "results/week20/e2e_retrieval_summary.json"

INFER_READY = {
    "backbone_raw": ROOT / "dataset/setting_a/29_n2_e2e_infer_ready/backbone_raw/test.json",
    "soft_support_raw": ROOT / "dataset/setting_a/29_n2_e2e_infer_ready/soft_support_raw/test.json",
    "retrieval_main": ROOT / "dataset/setting_a/29_n2_e2e_infer_ready/retrieval_main/test.json",
}

OUT_MAIN = ROOT / "results/week20/e2e_main_table.json"
OUT_ABL = ROOT / "results/week20/e2e_ablation.json"
OUT_MD = ROOT / "reports/week20/day5_e2e_table_and_ablation.md"
...
def main() -> None:
    ...
    backbone_candidate = day3["backbone_raw"]["candidate_ceiling_reviewer_safe"]
    backbone_e2e = day3["backbone_raw"]["e2e_generation_reviewer_safe"]

    soft_candidate = day3["soft_support_raw"]["candidate_ceiling_reviewer_safe"]
    soft_e2e = day3["soft_support_raw"]["e2e_generation_reviewer_safe"]

    retrieval_candidate = day4["retrieval_main"]["candidate_ceiling_reviewer_safe"]
    retrieval_e2e = day4["retrieval_main"]["e2e_generation_reviewer_safe"]
    ...
    if (
        ...
        decision_note = (
            "Keep retrieval_main as the paper-facing main row: soft support provides the main "
    ...
    ablation = {
        ...
        "interpretation": {
            ...
            "known_limitation": (
                "E2E Hits@1 remains weak because the frozen LLM often generates a plausible candidate "
    ...
    md = []
    md.append("# Day 5 — E2E main table and ablation")
    md.append("")
