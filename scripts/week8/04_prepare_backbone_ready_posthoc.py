...
def convert_candidate_ready_to_ranked_input(
    ...
    for row in candidate_ready_rows:
        ...
        if len(rank_entities) != 20 or len(rank_entities_id) != 20:
            raise ValueError(
                f"Candidate list length must be 20, got "
                f"len(names)={len(rank_entities)}, len(ids)={len(rank_entities_id)} "
                f"for query={row['query_entity']}"
            )
...
def main():
    ...
    parser.add_argument("--config", required=True, help="Path to configs/week8/backbone_ready_posthoc_debias.yaml")
    args = parser.parse_args()

    cfg = PrepareConfig(**load_yaml(Path(args.config)))
    ...
    if cfg.target_relation not in relation2id:
        raise ValueError(f"target_relation '{cfg.target_relation}' not found in relation2id")
    ...
    train_lookup = build_split_lookup(Path(cfg.train_split_tsv), entity2id, relation2id, cfg.target_relation)
    valid_lookup = build_split_lookup(Path(cfg.valid_split_tsv), entity2id, relation2id, cfg.target_relation)
    test_lookup = build_split_lookup(Path(cfg.test_split_tsv), entity2id, relation2id, cfg.target_relation)
    ...
    prompt_log_path = report_dir / "day4_prepare_backbone_ready_posthoc.log"
    ...
    manifest = {
        ...
        "goal": "Build backbone-ready package from best_lambda_artifacts after post-hoc debias.",
        "config": asdict(cfg),
        ...
        "important_note": "This package uses post-hoc debiased candidate artifacts with best lambda.",
    }
    save_json(output_dir / "manifest.json", manifest)

    md_path = report_dir / "day4_backbone_ready_posthoc.md"
    with md_path.open("w", encoding="utf-8") as f:
        f.write("# Week8 — backbone-ready posthoc debias package\n\n")
        f.write("## Scope\n\n")
        f.write("- Convert best-lambda drkgc-ready candidates into ranked_input JSON.\n")
        f.write("- Reuse prompt_subgraph.py to build final train/valid/test.\n")
        f.write("- Preserve exact leakage checking.\n\n")
        f.write("## Outputs\n\n")
        f.write(f"- `{train_saved_json}`\n")
        f.write(f"- `{valid_saved_json}`\n")
        f.write(f"- `{test_saved_json}`\n")
        f.write(f"- `{output_dir / 'manifest.json'}`\n")
        f.write(f"- log: `{prompt_log_path}`\n")

    print("Saved:")
    print("-", train_saved_json)
    print("-", valid_saved_json)
    print("-", test_saved_json)
    print("-", output_dir / "manifest.json")
    print("-", md_path)
    print("-", prompt_log_path)
