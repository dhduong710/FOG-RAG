...
def write_error_cases_md(path: Path, error_buckets: Dict[str, List[Dict[str, Any]]]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", encoding="utf-8") as f:
        f.write("# Posthoc 3B valid error cases\n\n")
        for bucket in [
...
def write_day5_markdown(
    path: Path,
    metrics_posthoc: Dict[str, Any],
    metrics_week6: Dict[str, Any] | None,
    metrics_week7: Dict[str, Any] | None,
    metrics_week7_8b: Dict[str, Any] | None,
    candidate_report_posthoc: Dict[str, Any] | None,
    error_buckets: Dict[str, List[Dict[str, Any]]],
):
    lines = []
    lines.append("# Week8 — valid rerun on posthoc debias package (3B)")
    lines.append("")
    lines.append("## Current valid metrics")
    lines.append("")
    lines.append(f"- mrr = `{metrics_posthoc['mrr']}`")
    lines.append(f"- hits1 = `{metrics_posthoc['hits1']}`")
    lines.append(f"- hits3 = `{metrics_posthoc['hits3']}`")
    lines.append(f"- hits10 = `{metrics_posthoc['hits10']}`")
    lines.append(f"- num_examples = `{metrics_posthoc['num_examples']}`")
    lines.append("")

    if metrics_week6 is not None:
        lines.append("## Compare against week6 3B")
        lines.append("")
        for k in ["mrr", "hits1", "hits3", "hits10"]:
            lines.append(f"- week6_{k} = `{metrics_week6.get(k)}`")
            lines.append(f"- posthoc_{k} = `{metrics_posthoc.get(k)}`")
            lines.append(f"- delta_vs_week6_{k} = `{round(metrics_posthoc.get(k, 0.0) - metrics_week6.get(k, 0.0), 8)}`")
            lines.append("")

    if metrics_week7 is not None:
        lines.append("## Compare against week7 3B")
        lines.append("")
        for k in ["mrr", "hits1", "hits3", "hits10"]:
            lines.append(f"- week7_{k} = `{metrics_week7.get(k)}`")
            lines.append(f"- posthoc_{k} = `{metrics_posthoc.get(k)}`")
            lines.append(f"- delta_vs_week7_{k} = `{round(metrics_posthoc.get(k, 0.0) - metrics_week7.get(k, 0.0), 8)}`")
            lines.append("")

    if metrics_week7_8b is not None:
        lines.append("## Compare against week7 8B")
        lines.append("")
        for k in ["mrr", "hits1", "hits3", "hits10"]:
            lines.append(f"- week7_8b_{k} = `{metrics_week7_8b.get(k)}`")
            lines.append(f"- posthoc_3b_{k} = `{metrics_posthoc.get(k)}`")
            lines.append(f"- delta_vs_week7_8b_{k} = `{round(metrics_posthoc.get(k, 0.0) - metrics_week7_8b.get(k, 0.0), 8)}`")
            lines.append("")

    if candidate_report_posthoc is not None:
        v = candidate_report_posthoc["splits"]["valid"]
    ...
    lines.append("")
    lines.append("- This is the final clean check of whether post-hoc debias improves end-to-end reranker behavior.")
    lines.append("- If valid improves over week7 3B without hurting candidate metrics, keep post-hoc debias as the final retrieval package.")
    lines.append("- If valid stays flat or worsens, keep post-hoc debias only as a retrieval-side analysis result.")
    lines.append("")
...
def main():
    ...
    valid_eval_dir = output_dir / "valid_eval_posthoc_3b"
    valid_eval_dir.mkdir(parents=True, exist_ok=True)

    metrics_out = valid_eval_dir / "eval_valid_posthoc_3b_metrics.json"
    preds_out = valid_eval_dir / "eval_valid_posthoc_3b_prediction.json"
    error_md_out = valid_eval_dir / "eval_valid_posthoc_3b_error_cases.md"
    report_md = Path("reports/week8/day5_valid_posthoc_3b.md")
    ...
    metrics_posthoc, preds, error_buckets = run_valid_eval(
        model=model,
        tokenizer=tokenizer,
    ...
    save_json(metrics_out, metrics_posthoc)
    save_json(
        preds_out,
        {
            "args": {
            ...
            "metrics": metrics_posthoc,
            "prediction": preds,
        },
    )
    write_error_cases_md(error_md_out, error_buckets)

    cmp = cfg.get("compare_against", {})
    metrics_week6 = load_json(Path(cmp["week6_valid_metrics"])) if cmp.get("week6_valid_metrics") and Path(cmp["week6_valid_metrics"]).exists() else None
    metrics_week7 = load_json(Path(cmp["week7_valid_metrics"])) if cmp.get("week7_valid_metrics") and Path(cmp["week7_valid_metrics"]).exists() else None
    metrics_week7_8b = load_json(Path(cmp["week7_8b_valid_metrics"])) if cmp.get("week7_8b_valid_metrics") and Path(cmp["week7_8b_valid_metrics"]).exists() else None
    candidate_report_posthoc = load_json(Path(cmp["posthoc_candidate_report"])) if cmp.get("posthoc_candidate_report") and Path(cmp["posthoc_candidate_report"]).exists() else None

    write_day5_markdown(
        path=report_md,
        metrics_posthoc=metrics_posthoc,
        metrics_week6=metrics_week6,
        metrics_week7=metrics_week7,
        metrics_week7_8b=metrics_week7_8b,
        candidate_report_posthoc=candidate_report_posthoc,
