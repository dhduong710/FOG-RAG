...
SPLIT_DIR = SETTING_DIR / "02_splits"
GRAPH_DIR = SETTING_DIR / "03_graph"
BASELINE_DIR = SETTING_DIR / "04_baseline_outputs"

SOURCE_ROOT = SETTING_DIR / "05_backbone_raw_source"
READY_ROOT = SETTING_DIR / "06_fograg_ready"

RESULT_DIR = ROOT / "results" / "week28"
REPORT_DIR = ROOT / "reports" / "week28"
...
def build_one_source(source_model, entity2id, id2entity, relation2id, id2relation, type_map, candidate_ids, adj, edge_set, kind_by_id, args):
    ...
    support_schema = {
        ...
        "note": "repoDB FOG-RAG-ready package. Diseases are repoDB-local UMLS nodes; drug/gene evidence is reused from DRKG where possible."
    }
    ...
    if not all(a["schema_pass"] for a in audit.values()):
        decision = "DAY5_REPODB_FOGRAG_NEEDS_FIX"
...
def write_report(summary: dict[str, Any]):
    path = REPORT_DIR / "day5_repodb_fograg_ready.md"

    lines = []
    lines.append("# Week 28 Day 5  repoDB FOG-RAG-ready packages")
    lines.append("")
...
def main():
    ...
    decision = "DAY5_REPODB_FOGRAG_READY"
    if not all(m["decision"] == "DAY5_REPODB_FOGRAG_READY" for m in manifests.values()):
        decision = "DAY5_REPODB_FOGRAG_PARTIAL_OR_NEEDS_FIX"
    ...
    write_json(summary, RESULT_DIR / "day5_repodb_fograg_ready_summary.json")
