...
mkdir -p logs/week28 results/week28/e2e_repodb_rgcn
...
run_row () {
  ...
  local out_dir="results/week28/e2e_repodb_rgcn/${row_name}"
  ...
  if [[ -f "${ckpt}/adapter_config.json" && -f "${ckpt}/graph_model.bin" ]]; then
    ...
    python main.py \
      ...
      --report_to none \
      --seed "${SEED}" \
      2>&1 | tee "logs/week28/day8_train_${row_name}.log"
  fi
  ...
  for split in valid test; do
    ...
    python infer.py \
      ...
      2>&1 | tee "logs/week28/day8_infer_${row_name}_${split}.log"
  done
}

run_row "backbone_raw" "dataset/setting_f_repodb/09_e2e_soft_support_ready/rgcn_raw_display_control"
run_row "soft_support_sweep" "dataset/setting_f_repodb/09_e2e_soft_support_ready/rgcn_sweep_selected"
run_row "fuzzy_retrieval_main" "dataset/setting_f_repodb/11_e2e_fuzzy_retrieval_ready/rgcn"
