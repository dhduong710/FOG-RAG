...
def write_error_md(path: Path, error_buckets: Dict[str, List[Dict[str, Any]]]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    with path.open("w", encoding="utf-8") as f:
        f.write("# Valid eval 8B error cases\n\n")
        for bucket in [
...
def write_markdown_report(
    ...
    metrics_week7_3b: Dict[str, Any] | None,
    candidate_report_week7: Dict[str, Any] | None,
    error_buckets: Dict[str, List[Dict[str, Any]]],
):
    lines = []
    lines.append("# 8B valid rerun report")
    lines.append("")
    lines.append("## Current 8B valid metrics")
    lines.append("")
    lines.append(f"- mrr = `{metrics_8b['mrr']}`")
    lines.append(f"- hits1 = `{metrics_8b['hits1']}`")
    lines.append(f"- hits3 = `{metrics_8b['hits3']}`")
    lines.append(f"- hits10 = `{metrics_8b['hits10']}`")
    lines.append(f"- num_examples = `{metrics_8b['num_examples']}`")
    lines.append("")
    ...
    if metrics_week6 is not None:
        ...
        for k in ["mrr", "hits1", "hits3", "hits10"]:
            ...
            lines.append(f"- current_8b_{k} = `{metrics_8b.get(k)}`")
            lines.append(f"- delta_vs_week6_{k} = `{round(metrics_8b.get(k, 0.0) - metrics_week6.get(k, 0.0), 8)}`")
            lines.append("")

    if metrics_week7_3b is not None:
        lines.append("## Compare against week7 3B rerun")
        lines.append("")
        for k in ["mrr", "hits1", "hits3", "hits10"]:
            lines.append(f"- week7_3b_{k} = `{metrics_week7_3b.get(k)}`")
            lines.append(f"- current_8b_{k} = `{metrics_8b.get(k)}`")
            lines.append(f"- delta_vs_week7_3b_{k} = `{round(metrics_8b.get(k, 0.0) - metrics_week7_3b.get(k, 0.0), 8)}`")
            lines.append("")
    ...
    lines.append("")
    lines.append("- This 8B run is a capacity sanity check on the same week7 candidate package.")
    lines.append("- If 8B improves clearly over week7 3B, backbone capacity is part of the bottleneck.")
    lines.append("- If 8B still struggles badly, candidate collapse/retrieval quality remains the dominant issue.")
    lines.append("")
...
def main():
    ...
    valid_eval_dir = output_dir / "valid_eval_8b"
    valid_eval_dir.mkdir(parents=True, exist_ok=True)

    metrics_out = valid_eval_dir / "eval_valid_8b_metrics.json"
    preds_out = valid_eval_dir / "eval_valid_8b_prediction.json"
    error_md_out = valid_eval_dir / "eval_valid_8b_error_cases.md"
    report_md = Path("reports/week7/day6_8b_valid_eval.md")
    ...
    metrics_8b, preds, error_buckets = run_valid_eval(
        model=model,
        tokenizer=tokenizer,
    ...
    save_json(metrics_out, metrics_8b)
    save_json(
        preds_out,
        {
            "args": {
    ...
    write_error_md(error_md_out, error_buckets)
    ...
    metrics_week7_3b = load_json(Path(cmp["week7_3b_valid_metrics"])) if cmp.get("week7_3b_valid_metrics") and Path(cmp["week7_3b_valid_metrics"]).exists() else None
    candidate_report_week7 = load_json(Path(cmp["week7_candidate_report"])) if cmp.get("week7_candidate_report") and Path(cmp["week7_candidate_report"]).exists() else None

    write_markdown_report(
        path=report_md,
        metrics_8b=metrics_8b,
        metrics_week6=metrics_week6,
        metrics_week7_3b=metrics_week7_3b,
