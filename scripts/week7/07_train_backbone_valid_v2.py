...
"""
Week 7 - Day 6
Train a short backbone rerun on dataset/setting_a/12_backbone_ready_ranker_v2
using the same backbone path as earlier weeks, but with improved candidate source.

This script is a wrapper around main.py.
"""
...
def main():
    ...
    log_path = Path("reports/week7/day6_train_backbone_v2.log")
    ...
    if proc.returncode != 0:
        raise RuntimeError(
            f"Backbone rerun training failed with return code {proc.returncode}. "
            f"See log: {log_path}"
        )
