...
class TransEModel(nn.Module):
    ...
    def score_triples(self, h_idx, r_idx, t_idx):
        h = self.dropout(self.ent(h_idx))
        r = self.dropout(self.rel(r_idx))
        t = self.dropout(self.ent(t_idx))
        return -torch.norm(h + r - t, p=self.p_norm, dim=-1)

    def score_head_candidates(self, relation_id, tail_ids, candidate_head_ids):
        h = self.ent(candidate_head_ids)               # [U, D]
        r = self.rel(torch.tensor([relation_id], device=tail_ids.device)).squeeze(0)  # [D]
        t = self.ent(tail_ids)                        # [B, D]
...
def compute_probe_metrics(model, valid_queries, indication_rel_id, drug_universe_ids, id2entity, device, max_queries=None):
    model.eval()
    rows = valid_queries[:max_queries] if (max_queries and max_queries > 0) else valid_queries
    with torch.no_grad():
        ...
        scores = model.score_head_candidates(indication_rel_id, tail_ids, cand_ids).detach().cpu().numpy()
...
def train_one_epoch(model, triples, optimizer, num_entities, device, batch_size, num_negatives, margin):
    model.train()
...
def main():
    ...
    parser.add_argument("--out_dir", default="results/week8/transe_baseline_full_v1")
    ...
    log_path = out_dir / "transe_train_log.jsonl"
    ckpt_path = out_dir / "transe_checkpoint.pt"
    meta_path = out_dir / "transe_meta.json"
    ...
    meta = {
        "method": "TransE",
        "goal": "Bonus structure-only TransE baseline for Setting A valid-first table v0",
        "seed": args.seed,
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        probe_metrics = compute_probe_metrics(
            ...
            id2entity=id2entity,
            device=device,
            max_queries=args.valid_probe_size,
        )
