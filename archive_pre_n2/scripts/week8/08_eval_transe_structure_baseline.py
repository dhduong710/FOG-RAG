...
class TransEModel(nn.Module):
    def __init__(self, num_entities, num_relations, embedding_dim=200, p_norm=1, dropout=0.1):
        super().__init__()
    ...
    def score_head_candidates(self, relation_id, tail_ids, candidate_head_ids):
        h = self.ent(candidate_head_ids)
        r = self.rel(torch.tensor([relation_id], device=tail_ids.device)).squeeze(0)
        t = self.ent(tail_ids)
        scores = -torch.norm(h.unsqueeze(0) + r.view(1, 1, -1) - t.unsqueeze(1), p=self.p_norm, dim=-1)
        return scores
...
def main():
    ...
    parser.add_argument("--checkpoint_path", default="results/week8/transe_baseline_full_v1/transe_checkpoint.pt")
    parser.add_argument("--meta_path", default="results/week8/transe_baseline_full_v1/transe_meta.json")
    ...
    torch.save(scores.detach().cpu(), out_dir / "transe_valid_scores.pt")
    save_json(out_dir / "transe_valid_metrics.json", metrics)
    save_json(out_dir / "transe_valid_case_samples.json", case_samples)

    with (out_dir / "transe_valid_top1_frequency.tsv").open("w", encoding="utf-8") as f:
    ...
    # append/update short report
    lines = []
    lines.append("# Day 5 ComplEx / TransE")
    lines.append("")
    lines.append("## 1. TransE main metrics")
    lines.append(f"- MRR: {metrics['mrr']}")
    lines.append(f"- Hits@1: {metrics['hits1']}")
    lines.append(f"- Hits@3: {metrics['hits3']}")
    lines.append(f"- Hits@10: {metrics['hits10']}")
    lines.append("")
    lines.append("## 2. Supporting analysis")
    lines.append(f"- top1_dominance_ratio: {metrics['top1_dominance_ratio']}")
    lines.append(f"- unique_top1_count: {metrics['unique_top1_count']}")
    lines.append("")
    lines.append("## 3. Decision")
    lines.append("- TransE is a bonus fourth baseline.")
    lines.append("- ComplEx remains the preferred third baseline.")
    report_path.write_text("\n".join(lines), encoding="utf-8")

    print("Saved:")
    print(f"- {out_dir / 'transe_valid_scores.pt'}")
    print(f"- {out_dir / 'transe_valid_metrics.json'}")
    print(f"- {out_dir / 'transe_valid_top1_frequency.tsv'}")
    print(f"- {out_dir / 'transe_valid_case_samples.json'}")
    print(f"- {report_path}")
    print()
    print("Main metrics:")
    print(json.dumps({
