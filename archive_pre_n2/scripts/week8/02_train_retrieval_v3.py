...
class TrainConfig:
    ...
    train_hard_negative_pools_path: str
    valid_hard_negative_probe_path: str
...
def build_pool_map(pool_payload: Dict[str, Any]) -> Dict[Tuple[int, int], Dict[str, Any]]:
    out = {}
    for row in pool_payload["rows"]:
        key = (int(row["query_entity_id"]), int(row["gold_entity_id"]))
        out[key] = row
    return out


def sample_mixed_negatives_for_batch(
    *,
    batch_examples: List[Dict[str, Any]],
    train_pool_map: Dict[Tuple[int, int], Dict[str, Any]],
    ...
    """
    Returns neg_ids tensor [B, K] and batch sampling debug rows.
    """
    ...
    for ex in batch_examples:
        ...
        pool = train_pool_map[key]

        raw_ids = [int(x) for x in pool["hard_negative_entity_ids"]]
        raw_names = list(pool["hard_negative_entities"])

        # reconstruct sources
        selected = []
        selected_set = set()

        # A. query-specific raw negatives first
        raw_take = 0
        for nid in raw_ids:
        ...
        # Final crop if somehow too many
        selected = selected[:num_negatives]

        if len(selected) < num_negatives:
            raise ValueError(
                f"Not enough negatives after sampling for query={ex['disease_name']} gold={ex['drug_name']}. "
                f"len(selected)={len(selected)} expected={num_negatives}"
            )

        all_neg_ids.append(selected)
        debug_rows.append({
    ...
    return torch.tensor(all_neg_ids, dtype=torch.long, device=device), debug_rows
...
def run_valid_probe(
    ...
    for ex in probe_examples:
        ...
        scores = model.score_candidates_for_query(node_repr, cand_ids, relation_id, disease_id)
        topk = min(20, cand_ids.size(0))
        _, top_idx = torch.topk(scores, k=topk, largest=True, sorted=True)
        top_ids = cand_ids[top_idx].detach().cpu().tolist()

        if gold_id in top_ids:
            recall20 += 1
        if top_ids and top_ids[0] == gold_id:
            top1_hit += 1
        if top_ids:
...
def write_markdown_report(
    ...
    lines = []
    lines.append("# Week8 / retrieval-v3 Day 2 — ranker_v3 training")
    lines.append("")
    lines.append("## Scope")
    lines.append("")
    lines.append("- Train ranker_v3 with mixed negatives.")
    lines.append("- Reduce collapse by combining query-specific raw negatives with a small quota of collapse and bias-heavy drugs.")
    lines.append("")
    ...
    lines.append("")
    lines.append("- If valid_probe_recall20 improves and top1_dominance_ratio drops versus ranker_v2, the mixed-negative strategy is promising.")
    lines.append("- Day 3 will be the real scientific check by building candidate artifacts on full valid.")
    lines.append("")
...
def main():
    ...
    parser.add_argument("--config", required=True, help="Path to configs/week8/retrieval_v3_day2.yaml")
    ...
    train_log_path = output_dir / "ranker_v3_train_log.jsonl"
    ckpt_path = output_dir / "rgcn_ranker_v3_checkpoint.pt"
    meta_path = output_dir / "ranker_v3_meta.json"
    sampled_train_neg_path = output_dir / "sampled_train_negatives.json"
    sampled_valid_neg_path = output_dir / "sampled_valid_probe_negatives.json"
    score_stats_path = report_dir / "day2_ranker_v3_score_stats.json"
    top1_probe_tsv_path = report_dir / "day2_ranker_v3_top1_probe.tsv"
    day2_md_path = report_dir / "day2_ranker_v3_train.md"
    ...
    # prep artifacts
    train_pools_payload = load_json(Path(cfg.train_hard_negative_pools_path))
    valid_probe_pools_payload = load_json(Path(cfg.valid_hard_negative_probe_path))
    collapse_summary = load_json(Path(cfg.collapse_summary_path))
    drug_bias_stats = load_json(Path(cfg.drug_bias_stats_path))

    train_pool_map = build_pool_map(train_pools_payload)
    valid_pool_map = build_pool_map(valid_probe_pools_payload)
    ...
    sampled_valid_examples_debug = []

    def composite_probe_score(probe, avg_epoch_loss):
        return (
            1000.0 * probe["recall20"]
            - 12.0 * probe["top1_dominance_ratio"]
            + 4.0 * probe["unique_top1_count"]
            - 0.1 * avg_epoch_loss
        )
    ...
    for epoch in range(1, cfg.num_epochs + 1):
        ...
        for start in range(0, num_train, cfg.batch_size):
            ...
            neg_ids, debug_rows = sample_mixed_negatives_for_batch(
                batch_examples=batch_examples,
                train_pool_map=train_pool_map,
            ...
            if len(sampled_train_examples_debug) < 20:
                sampled_train_examples_debug.extend(debug_rows[: max(0, 20 - len(sampled_train_examples_debug))])
    ...
    # sample valid negatives for debug based on best setup
    probe_rows = valid_probe_pools_payload["rows"][:20]
    sampled_valid_examples_debug = probe_rows
    ...
    save_json(sampled_valid_neg_path, {
        "summary": {
            "num_saved_debug_rows": len(sampled_valid_examples_debug),
        },
        "rows": sampled_valid_examples_debug,
    })
    ...
    save_json(meta_path, {
        ...
        "stage": "retrieval_v3",
        "day": 2,
        "goal": "Train ranker_v3 with mixed hard negatives to reduce candidate collapse.",
        "config": asdict(cfg),
        "num_nodes": num_nodes,
        ...
        "important_note": "Day 2 only checks training/probe behavior; full candidate artifact comparison belongs to Day 3 retrieval-v3.",
    })
